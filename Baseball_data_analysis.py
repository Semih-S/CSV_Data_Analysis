...
def filter_by_year(statistics, year, yearid):
    ...
    """
    return [value for value in statistics if value[yearid] == str(year)]
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for player in statistics:
        all_players.append((player["player"], formula(info, player)))
    all_players.sort(key = lambda x: x[1], reverse=True)
    return all_players[:numplayers]
    ...
    for ids in top_ids_and_stats:
        for stat in statistics:
            if stat[info['playerid']] == ids[0]:
                list_of_strings.append(f'{ids[1]:.3f} --- {stat[info["firstname"]]} {stat[info["lastname"]]}')
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    with open(info["battingfile"], newline='', encoding="utf-8") as csvfile:
        reader = csv.DictReader(csvfile, delimiter=info['separator'], quotechar=info['quote'])
        stats = filter_by_year(reader, year, info['yearid'])
        
        top_player_ids_and_stats = top_player_ids(info, stats, formula, numplayers)
        return lookup_player_names(info, top_player_ids_and_stats)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for stat in statistics:
        if stat[playerid] in aggregated_players:
            ...
            aggregated_players[stat[playerid]] = {playerid: stat[playerid]}
            for field in fields:                
                if field in stat:
                    aggregated_players[stat[playerid]][field] = int(stat[field])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    bat_stat = [{k.lower(): v for k, v in d.items()} for d in bat_stat]
